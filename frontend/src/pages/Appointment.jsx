  ...
  const getAvailableSlots = async () =>{
               ...
               for(let i=0; i<7; i++){
                ...
                if(today.getDate()===currrentDate.getDate()){
                   currrentDate.setHours(currrentDate.getHours() ? currrentDate.getHours() +1 :10)
                ...
                while(currrentDate<endTime){
                  ...
                  //increament time by the 30 minute
       ...
       <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
        {
          docSlots.length && docSlots[slotIndex].map((item,index)=>(
            <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white':'text-gray-400 border border-gary-300'}`} key={index}> 
